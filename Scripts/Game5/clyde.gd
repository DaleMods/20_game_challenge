...
func get_scatter_position():
	return game.level.get_clyde_scatter()
...
func get_direction():
	...
	if is_chasing:
		if position.distance_to(get_chase_position()) > 160:
			target_pos = get_chase_position()
	if is_eyes:
